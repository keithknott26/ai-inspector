...
//
// AI Inspector analysis engine (epan plugin).
//
...
// as filterable ai_inspector.* fields, through `tshark -z ai_inspector,report|json`,
// and to the Qt UI plugin through the "ai_inspector" tap (see ai_inspector_api.h).
//
// Life of a frame
// ---------------
//  1. init_routine() runs when a capture is opened or redissected: it resets the
//     State, bumps the generation counter and asks epan to prime the fields in
//     AI_INSPECTOR_FIELDS (set_postdissector_wanted_hfids), so their values are
//     available even when no visible protocol tree is built.
//  2. dissect_ai_inspector() is called for every frame after all other
//     dissectors. On the first pass it runs Analyzer::run(), which records
//     findings into State. On later passes (GUI redraws, filtering) it only
//     renders the cached findings, so results never depend on pass order.
//  3. A tap record is queued so listeners (the UI plugin, -z ai_inspector) can
//     reach the engine API table.
//
// Adding a new check
// ------------------
//  1. If you need a field that is not listed yet, add it to AI_INSPECTOR_FIELDS
//     (name must be the exact Wireshark display-filter field name; missing
//     fields in a given Wireshark build are tolerated and reported).
//  2. Add the logic to the matching Analyzer method (or a new one called from
//     Analyzer::run()). Read values with P.u() / P.s() / P.d() / P.has().
//  3. Call add(severity, category, "PROTO", "proto.stable_id", "Title",
//     detail, "display filter that isolates the related packets").
//     IDs are part of the public interface (filters, tests, AI context):
//     keep them lowercase, dotted and stable.
//  4. Keep per-capture memory bounded (use bump()/once(), never unbounded maps).
//  5. Add a packet to tools/make-test-captures.py and the expected ID to
//     tests/run_tests.py (EXPECT), then run the integration tests.
//
// Threading: Wireshark dissects on one thread; g_mutex additionally guards State
// because the UI plugin calls the API functions from the GUI thread.
#include "config.h"
...
const char *const ENGINE_VERSION = AI_INSPECTOR_VERSION;
...
int proto_ai_inspector = -1;
int hf_count = -1, hf_finding = -1, hf_id = -1, hf_severity = -1, hf_category = -1;
int hf_protocol = -1, hf_detail = -1, hf_filter = -1;
int ett_ai_inspector = -1, ett_finding = -1;
int ai_inspector_tap = -1;
dissector_handle_t ai_inspector_handle;
...
// ------------------------------------------------------------------ analyzer
// One Analyzer is created per frame on the first pass. Each protocol family has
// its own method; run() calls them in order. Methods must be side-effect free
// apart from add()/bump()/once() and the heuristic state in State, and must
// tolerate any field being absent (malformed or truncated packets).
class Analyzer {
public:
...
std::string build_report(const State &S) {
    ...
    line(fmt("AI Inspector %s -- capture findings report", ENGINE_VERSION));
    line(std::string(78, '='));
    ...
    line("");
    line("Filters: ai_inspector.severity >= 3 | ai_inspector.category == \"security\" | ai_inspector.id == \"<id>\"");
...
std::string build_summary_json(const State &S, uint32_t max_findings) {
    JsonWriter w;
    w.begin_object();
    w.key("engine").str(std::string("AI Inspector ") + ENGINE_VERSION);
...
const ai_inspector_engine_api_t g_api = {AI_INSPECTOR_API_ABI, AI_INSPECTOR_VERSION, api_generation, api_summary_json,
...
void render(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, const std::vector<Finding> &list) {
    ...
    proto_item *root = proto_tree_add_protocol_format(tree, proto_ai_inspector, tvb, 0, 0, "AI Inspector: %zu finding%s (worst: %s)",
                                                      shown.size(), shown.size() == 1 ? "" : "s", severity_name(worst));
    proto_tree *rt = proto_item_add_subtree(root, ett_ai_inspector);
...
// Post-dissector entry point (registered in proto_reg_handoff_ai_inspector_engine).
int dissect_ai_inspector(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *) {
    ...
    if (have_tap_listener(ai_inspector_tap)) {
        auto *td = wmem_new0(pinfo->pool, ai_inspector_tap_data_t);
        td->abi = AI_INSPECTOR_API_ABI;
        td->frame = pinfo->num;
        td->worst_severity = worst;
        td->first_pass = first_pass;
        td->api = &g_api;
        tap_queue_packet(ai_inspector_tap, pinfo, td);
    }
...
void init_routine(void) {
    ...
    set_postdissector_wanted_hfids(ai_inspector_handle, wanted);
}

void cleanup_routine(void) {
    set_postdissector_wanted_hfids(ai_inspector_handle, nullptr);
}

// ------------------------------------------------------------------ tshark -z ai_inspector,report|json
struct CliTap {
    bool json;
};
...
bool cli_init(const char *opt_arg, void *) {
    // opt_arg is "ai_inspector,report" or "ai_inspector,json"
    const char *rest = std::strchr(opt_arg, ',');
    std::string mode = rest ? rest + 1 : "report";
    if (mode != "report" && mode != "json") {
        std::fprintf(stderr, "tshark: invalid -z ai_inspector mode \"%s\" (use report or json)\n", mode.c_str());
        return false;
    }
    auto *t = new CliTap{mode == "json"};
    GString *err = register_tap_listener(AI_INSPECTOR_TAP_NAME, t, nullptr, 0, nullptr, cli_packet, cli_draw, cli_finish);
    if (err) {
        std::fprintf(stderr, "tshark: couldn't register ai_inspector tap: %s\n", err->str);
...
stat_tap_ui g_cli_ui = {REGISTER_PACKET_ANALYZE_GROUP_UNSORTED, "AI Inspector findings", "ai_inspector", cli_init, 0, nullptr};

} // namespace

void proto_register_ai_inspector_engine(void) {
    static hf_register_info hf[] = {
        {&hf_count, {"Findings", "ai_inspector.count", FT_UINT16, BASE_DEC, nullptr, 0x0, "Number of findings in this frame", HFILL}},
        {&hf_finding, {"Finding", "ai_inspector.finding", FT_STRING, BASE_NONE, nullptr, 0x0, nullptr, HFILL}},
        {&hf_id, {"Finding ID", "ai_inspector.id", FT_STRING, BASE_NONE, nullptr, 0x0, nullptr, HFILL}},
        {&hf_severity, {"Severity", "ai_inspector.severity", FT_UINT8, BASE_DEC, VALS(severity_vals), 0x0, nullptr, HFILL}},
        {&hf_category, {"Category", "ai_inspector.category", FT_STRING, BASE_NONE, nullptr, 0x0, nullptr, HFILL}},
        {&hf_protocol, {"Protocol", "ai_inspector.protocol", FT_STRING, BASE_NONE, nullptr, 0x0, nullptr, HFILL}},
        {&hf_detail, {"Detail", "ai_inspector.detail", FT_STRING, BASE_NONE, nullptr, 0x0, nullptr, HFILL}},
        {&hf_filter, {"Related filter", "ai_inspector.filter", FT_STRING, BASE_NONE, nullptr, 0x0, nullptr, HFILL}},
    };
    static int *ett[] = {&ett_ai_inspector, &ett_finding};
    ...
    for (int c = 0; c < CAT_COUNT; ++c) {
        for (int s = SEV_INFO; s <= SEV_ERROR; ++s) {
            int i = c * 4 + (s - 1);
            names[i] = fmt("ai_inspector.%s.%s", CATEGORY_NAMES[c], lower(severity_name(s)).c_str());
            summaries[i] = fmt("AI Inspector %s %s", CATEGORY_NAMES[c], lower(severity_name(s)).c_str());
            ei_register_info e = {&g_ei[c][s], {names[i].c_str(), groups[c], sevs[s], summaries[i].c_str(), EXPFILL}};
            ei[i] = e;
        }
    }

    proto_ai_inspector = proto_register_protocol("AI Inspector", "AI_INSPECTOR", "ai_inspector");
    proto_register_field_array(proto_ai_inspector, hf, G_N_ELEMENTS(hf));
    proto_register_subtree_array(ett, G_N_ELEMENTS(ett));
    expert_module_t *em = expert_register_protocol(proto_ai_inspector);
    expert_register_field_array(em, ei, G_N_ELEMENTS(ei));

    module_t *m = prefs_register_protocol(proto_ai_inspector, nullptr);
    ...
    ai_inspector_handle = register_dissector("ai_inspector", dissect_ai_inspector, proto_ai_inspector);
    register_init_routine(init_routine);
    register_cleanup_routine(cleanup_routine);
    ai_inspector_tap = register_tap(AI_INSPECTOR_TAP_NAME);
    for (int &id : g_hf_ids) id = -1;
}

void proto_reg_handoff_ai_inspector_engine(void) {
    register_postdissector(ai_inspector_handle);
